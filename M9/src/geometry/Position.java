...
public class Position {
    ...
    public static Position barycenter(Collection<Position> positions) {
        if(positions == null){
            throw new java.lang.IllegalArgumentException("null pointer exception for positions in Position.barycenter");
        }
        if(positions.isEmpty()){
            throw new java.lang.IllegalArgumentException("positions is empty in Position.barycenter");
        }
        double x = 0;
        double y = 0;
        int numberOfPoints = positions.size();
        for (Position pos: positions) {
            x += pos.getX();
            y+= pos.getY();
        }

        return new Position(x/numberOfPoints,y/numberOfPoints);
    }
