...
public class Position {
   ...
   static public double euclide(Position p1, Position p2){
      double deltaX = p2.x - p1.x;
      double deltaY = p2.y - p1.y;
      return Math.sqrt(deltaX * deltaX + deltaY * deltaY);
   }

   static public double manhattan(Position p1, Position p2){
      return Math.abs(p2.x - p1.x) + Math.abs(p2.y - p1.y);
   }

   public enum DistanceType {euclide,manhattan};

   static public double distance(DistanceType type, Position p1, Position p2){
      switch(type) {
         case euclide: return euclide(p1, p2);
         case manhattan: return manhattan(p1, p2);
         default: return 0;
      }
   }
}
